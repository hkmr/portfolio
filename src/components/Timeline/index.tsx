    ...
    <div>
      <ol className="items-center sm:flex">
        {data.items.map((item, index) => {
          return <TimelineItem key={index} title={item.title} subtitle={item.subtitle} date={item.date} description={item.description} logo={item.logo} />;
        })}
      </ol>
    </div>
  );
...
const TimelineItem = ({ title, subtitle, date, description, logo }: { title: string; subtitle: string; date: string; description: string; logo?: string }) => {
  return (
      ...
      <div className="flex items-center">
        <div className="z-10 flex items-center justify-center w-6 h-6 bg-white rounded-full ring-0 ring-gray sm:ring-8 shrink-0">
          {logo ? (
            ...
            <svg className="w-3 h-3 text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
